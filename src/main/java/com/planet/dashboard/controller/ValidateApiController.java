...
public class ValidateApiController {
    ...
    public Header<ValidationResponse> validateNickName(@RequestBody NickNameValidationRequest nickNameValidationRequest , HttpSession session){
        ValidationResponse response = new ValidationResponse();
        if(isExistNickName(nickNameValidationRequest.getNickName())){
            response.setDuplicate("중복된 닉네임입니다. 다른 닉네임을 입력해주세요.");
            return Header.response(response);
        }
        SessionManager.addSession(session,SessionManager.NICKNAME_AUTH , nickNameValidationRequest.getNickName());
